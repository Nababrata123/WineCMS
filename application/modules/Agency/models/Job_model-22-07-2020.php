...
class Job_model extends CI_Model {
    ...
    public function get_job_list($filter = array(), $order = null, $dir = null, $count = false) {
        ...
        $this->db->select('job.*,store.name as store_name, store.adress as address, store.state as state, store.city as city, store.zipcode as zipcode');
        $this->db->from('job');
        ...
        if (isset($filter['search_text']) && $filter['search_text'] != "~" && $filter['search_text'] != "") {
            ...
            $this->db->where("((store.name LIKE '%$searchText%') OR (store.adress LIKE '%$searchText%') OR (store.state LIKE '%$searchText%') OR (store.city LIKE '%$searchText%') OR (store.zipcode LIKE '%$searchText%'))");
        }
