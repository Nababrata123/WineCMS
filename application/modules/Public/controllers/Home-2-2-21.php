...
class Home extends Front_Controller {
	...
	*/
	...
	public function job_rating(){
		...
		$data['rating'] = $this->uri->segment(3);
       ...
       $salesrep_name = $salesrep->sales_rep_name;
		...
		$m_result=$this->db->get()->row_array();
			...
			if ($this->db->insert('job_rating', $insData)) {
				// $this->session->set_flashdata('message_type', 'success');
				   // $this->session->set_flashdata('message','Operation successfully');

				//$dataforsalesrep = $this->jobRatingMailTemplate($job_id, $m_result['first_name'], $tastingDate, $data['tasterName'], $data['job_start_time'], $data['finish_time'], $data['wineNames'],  $stars, $feedback);
				...
				//$dataforadmin = $this->jobRatingMailTemplate($job_id, 'Admin', $tastingDate, $data['tasterName'], $data['job_start_time'],$data['finish_time'], $data['wineNames'],  $stars, $feedback);
				...
				//$this->email_to_user('rr.avalgate@gmail.com', 'Wine Sampling - '.$tastingDate, $dataforadmin); //admin mail 
				$this->email_to_user('fraidy@thekgroupny.com', 'Wine Sampling - '.$tastingDate, $dataforadmin); //admin mail 
				//$this->email_to_user('tastingresults@gmail.com', 'Wine Sampling - '.$tastingDate, $dataforadmin); //admin mail
	...
	public function jobRatingMailTemplate($job_id, $manager_name, $samplingDate, $tasterName, $startTime, $finish_time, $wineNames, $store_name, $store_address, $salesrep_name, $starscount, $feedback)
    {
                    ...
                    <div>
                        <table class="container">
                          ...
                          <tr>
							...
							<tr>
								<td colspan="3">
									<h4>Tasting Date - '.$samplingDate.'</h4>
								</td>
							</tr>
                            <tr>
                                <td width="100">
                                <h4 style="margin-top:auto;" class="tim">Job start time - '.date("g:i a", strtotime($startTime)).'</h4>
                                </td>
                                <td width="100">
                                <h4 style="margin-top:auto;" class="tim">Job end time - '.date("g:i a", strtotime($finish_time)).'</h4>
                                </td>
                            </tr>
                            <tr>
                                <td colspan="3">
