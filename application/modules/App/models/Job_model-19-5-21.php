...
class Job_model extends CI_Model {
    ...
    public function get_billing_list($filter = array(), $order = null, $dir = null, $count = false) {
        ...
        if (isset($filter['search_text']) && $filter['search_text'] != "~" && $filter['search_text'] != "" && !$count)  {
        
              $searchUrlText = urldecode($filter['search_text']);
            //   $searchText = base64_decode($filter['search_text']);
            //   $searchView = $filter['view'];

            //   $searchText = $filter['search_text'];

            //   print_r($filter['search_text']);
            //   print_r(base64_encode($filter['search_text']));
            //   print_r($searchUrlText);

            //   print_r($searchText);die;

            //   print_r($searchView);die;
            ...
            //   }else{
            //     $this->db->where("( (store.name LIKE '%$searchText%') OR (agency_name.meta_value LIKE '%$searchText%') OR (CONCAT(taster.last_name,' ',taster.first_name) LIKE '%$searchText%') OR (CONCAT(agency_taster.last_name,' ',agency_taster.first_name) LIKE '%$searchText%') OR (CONCAT(sales_rep.last_name,' ',sales_rep.first_name) LIKE '%$searchText%') )");
            //   }
