...
class Wine_model extends CI_Model {
    ...
    public function search_wine($search_text,$wine_sell_type,$serverTime)
    {
        

        $this->db->select("wine.id as id,wine.name as name,wine.description,UPPER(wine.flavour)  as type,wine.company_type,wine.upc_code as upc,wine.brand,wine.year,wine.size as size,wine.category_id, UOM, wine.created_on, wine.updated_on, wine.is_deleted, wine.status");
        $this->db->from('wine');
        $this->db->join('category', 'category.id = wine.category_id OR wine.category_id = 0');
        ...
        else if(strtolower($wine_sell_type)=='kayco')
        {
            $this->db->where('wine.flavour','kayco');
        }
        else if(strtolower($wine_sell_type)=='other')
        {
            $this->db->where('wine.flavour','other');
        }
        
        // Update Or new created Wine list get..
        if ($serverTime !=''){
            $this->db->where('wine.updated_on >=', $serverTime);
            $this->db->or_where('wine.created_on >=', $serverTime); 
        }
        ...
        for($i=0;$i<count($wine_array);$i++)
        {
            ...
            $wine_array[$i]['size']= (double) $wine_array[$i]['size'].' '.$wine_array[$i]['UOM'];
            ...
            if($wine_array[$i]['status'] == 'inactive'){
                $wine_array[$i]['is_deleted'] = 1;
            }
    ...
    public function get_wine_list_for_csv($items_to_export)
    {
        ...
        $this->db->select('id,upc_code,name,brand,year,type,description,size,UOM,category_id,flavour as company');
        $this->db->from('wine');
    ...
    function checkDuplicateUpccodeReturnId($upc_code)
    {
		//echo $upc_code;die;
        $this->db->select('id, upc_code');
        $this->db->from('wine');
        $this->db->where('upc_code',$upc_code);
        $this->db->where('status','active');
        $this->db->where('is_deleted',0);
        ...
        if ($row) {
          //if count row return any row; that means you have already this email address in the database. so you must set false in this sense.
            return $wineId; // here I change TRUE to false.
         } else {
          // doesn't return any row means database doesn't have this email
            return TRUE; // And here false to TRUE
         }
    ...
    public function save_product_image($wine_id, $image) {

        $this->db->select('id, wine_id');
        $this->db->from('wine_images');
        $this->db->where('wine_images.wine_id',$wine_id);
        $this->db->where('is_deleted',0);
        $this->db->order_by("wine_images.id",'DESC');
        $value=$this->db->get();
        $wine_image_array = $value->result_array();

        if (count($wine_image_array) > 0 ){
            $lastImage = $wine_image_array[0];
            $imageId = $lastImage['id'];
            if($imageId != '') {
                // Update Image ...
                $data = array('wine_id' => $wine_id, 'image' => $image);     
                $this->Wine_model->update('wine_images', 'id', $imageId, $data);
            }else{
                //Insert Image ..
                $data = array('wine_id' => $wine_id, 'image' => $image);
                $this->db->insert('wine_images', $data);
    ...
    public function getAll_WineUOM() {

        $this->db->select('UOM');
        $this->db->from('wine');
        $this->db->where('is_deleted',0);
        $value=$this->db->get();
        $wine_UOM_array = $value->result_array();

        $data = array_map('unserialize', array_unique(array_map('serialize', $wine_UOM_array)));
        $sizeArray = array();
        foreach($data as $index){
            array_push($sizeArray,$index['UOM']);
        }
       
        return $sizeArray;
    }

    function checkValidCategoryName($categoryName){
