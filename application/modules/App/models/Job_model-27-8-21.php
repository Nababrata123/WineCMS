...
class Job_model extends CI_Model {
    ...
    function get_joblist_old($user_id)
    {
        //check user type
    ...
    {
        // print_r($serverTime);die;
        //check user type
        $user_info=$this->get_user_type('users',$user_id);
        $user_type=$user_info[0]->user_type;
        $user_created_by=$user_info[0]->created_by;
        
        $this->db->select("job.id as job_id,job.status,job.previous_taster_id,job.job_state,DATE_FORMAT(job.tasting_date, '%d-%m-%Y') as job_date,TIME_FORMAT(start_time, '%h:%i%p') as start_time,TIME_FORMAT(end_time, '%h:%i%p') as end_time,store.name as store_name,store.adress as address,store.suite_number as suite_number,store.appartment_number as appartment_number");
        $this->db->from('job');
        $this->db->join('store','job.store_id=store.id');
        ...
        else 
        {
            ...
            if($user_created_by==7)
            {
                ...
                // $this->db->or_where('job.previous_taster_id', $user_id);
                // $this->db->like('job.taster_id',$user_id);
                //$this->db->where('job.job_status <>',1);
            }
            else 
    ...
    function get_accepted_joblist($tester_id,$todays_job,$serverTime=NULL)
    {
        ...
        $this->db->select("job.id as job_id,job.status,job.previous_taster_id,job.job_state,DATE_FORMAT(job.tasting_date, '%d-%m-%Y') as job_date,TIME_FORMAT(start_time, '%h:%i%p') as start_time,TIME_FORMAT(end_time, '%h:%i%p') as end_time,store.name as store_name,store.adress as address,store.suite_number  as suite_and_apt");
        $this->db->from('job');
    ...
    function check_tester_availablity_new1($tablename,$taster_id,$tasting_date,$start_time,$end_time)
    {
        ...
        //     if (($start_timestamp >= $existing_start_timestamp) && ($start_timestamp <= $existing_end_timestamp)){
        //         echo "Current job is between two dates";die;
        //     }else{
        //         echo "Current job is not between two dates";die;  
        //     }
...
/*
			...
			$this->db->from($tablename);
			// $this->db->where('tasting_date',$tasting_date);
            $previousDay = date('Y-m-d', strtotime('-1 day', strtotime($tasting_date)));

            $where_2 = " (tasting_date = '$tasting_date' OR (`start_time` > `end_time` AND tasting_date = '$previousDay'))";
            ...
            if ($start_time > $end_time) {
                ...
                */
                /*
                $where_1 = " ( (`start_time` <= '".$start_time."' AND `end_time` + CAST('24:00:00' AS TIME) >= '".$end_time."')
                OR (`start_time` >= '".$start_time."'  AND `start_time` <= '".$end_time."' AND `end_time` + CAST('24:00:00' AS TIME) <= '".$end_time."')  
                OR (`end_time` + CAST('24:00:00' AS TIME) <= '".$end_time."' AND `end_time` + CAST('24:00:00' AS TIME) >= '".$start_time."'  AND `start_time` <= '".$start_time."' )  
                OR (start_time >= '".$start_time."'  AND start_time <= '".$end_time."') )";*/
/*
                $where_1 = "((`start_time` <= '".$start_time."' AND (CASE
                WHEN `tasting_date` = '".$tasting_date."' THEN `end_time` + CAST('24:00:00' AS TIME) ELSE `end_time` END) >= '".$end_time."') OR (`start_time` >= '".$start_time."' AND `start_time` <= '".$end_time."' AND (CASE
                 WHEN `tasting_date` = '".$tasting_date."' THEN `end_time` + CAST('24:00:00' AS TIME) ELSE `end_time` END ) <= '".$end_time."') OR ((CASE
                  WHEN `tasting_date` = '".$tasting_date."' THEN `end_time` + CAST('24:00:00' AS TIME) ELSE `end_time` END) <= '".$end_time."' AND (CASE
                   WHEN `tasting_date` = '".$tasting_date."' THEN `end_time` + CAST('24:00:00' AS TIME) ELSE `end_time` END) >= '".$start_time."' AND `start_time` <= '".$start_time."' ) OR (`start_time` >= '".$start_time."' AND `start_time` <= '".$end_time."')) ";
                ...
                $where_1 = "((`start_time` <= '".$start_time."' AND (CASE
                 WHEN `tasting_date` = '".$tasting_date."' THEN `end_time` ELSE `end_time` + CAST('24:00:00' AS TIME) END) >= '".$end_time."') OR (`start_time` >= '".$start_time."' AND `start_time` <= '".$end_time."' AND (CASE
                  WHEN `tasting_date` = '".$tasting_date."' THEN `end_time` ELSE `end_time` + CAST('24:00:00' AS TIME) END ) <= '".$end_time."') OR ((CASE
                   WHEN `tasting_date` = '".$tasting_date."' THEN `end_time` ELSE `end_time` + CAST('24:00:00' AS TIME) END) <= '".$end_time."' AND (CASE
                    WHEN `tasting_date` = '".$tasting_date."' THEN `end_time` ELSE `end_time` + CAST('24:00:00' AS TIME) END) >= '".$start_time."' AND `start_time` <= '".$start_time."' ) OR (`start_time` >= '".$start_time."' AND `start_time` <= '".$end_time."')) ";
            }
...
*/
    ...
    function check_tester_availablity_new($tablename,$taster_id,$tasting_date,$start_time,$end_time)
    {
        ...
        $nextDay = date('Y-m-d', strtotime('+1 day', strtotime($tasting_date)));
        $previousDay = date('Y-m-d', strtotime('-1 day', strtotime($tasting_date)));
        // $dateArray = array($nextDay, $tasting_date, $previousDay);

		if($user_type == 'tester'){

            $count = 0;
            if(($tasting_date > $today)){
      
            $this->db->select('COUNT(*) as count,id');
			$this->db->from($tablename);
            if ($time_one == 'PM' && $time_two == 'AM'  && $end_time <= '03:00'){
                $dateArray = array($nextDay, $tasting_date, $previousDay);
                $this->db->where_in('tasting_date',$dateArray);
            }else{
                $dateArray = array($tasting_date);
                $this->db->where_in('tasting_date',$dateArray);
            }
			
			$this->db->where('is_deleted',0);
			$this->db->where('status !=','completed');
            $this->db->where('status !=','problems');
            $this->db->where('status !=','cancelled');

            if ($time_one == 'PM' && $time_two == 'AM' && $end_time <= '03:00'){
                $where_1 = "((`start_time` <= '".$start_time."' AND `end_time` >= '".$end_time."' AND `end_time` >='".$start_time."'))";
            }else{
                $where_1 = " ( (`start_time` <= '".$start_time."' AND `end_time` >= '".$end_time."')
                OR (`start_time` >= '".$start_time."'  AND `start_time` <= '".$end_time."' AND `end_time` <= '".$end_time."')  
                OR (`end_time` <= '".$end_time."' AND `end_time` >= '".$start_time."'  AND `start_time` <= '".$start_time."' )  
            ...
            if ($time_one == 'PM' && $time_two == 'AM' && $end_time <= '03:00'){
                $where_1 = "((`start_time` <= '".$start_time."' AND `end_time` >= '".$end_time."' AND `end_time` >='".$start_time."')
                OR (`start_time` >= '".$start_time."' AND `end_time` <= '".$end_time."') OR (`start_time` >= '".$start_time."' AND `end_time` >= '".$end_time."' AND `end_time` <= '03:00') OR(`start_time` >= '".$start_time."'  AND `start_time` <= '".$end_time."' AND `end_time` <= '".$end_time."')  
                OR (`end_time` <= '".$end_time."' AND `end_time` >= '".$start_time."'  AND `start_time` <= '".$start_time."' )  
                OR (start_time >= '".$start_time."'  AND start_time <= '".$end_time."'))";
            }else{
                $where_1 = " ( (`start_time` <= '".$start_time."' AND `end_time` >= '".$end_time."')
...
//  echo $this->db->last_query();die;
    ...
    function check_tester_availablity_with_jobid_new($jobid,$tablename,$taster_id,$tasting_date,$start_time,$end_time)
    {
        $this->db->select('user_type');
        $this->db->from('users');
        $this->db->where('users.id',$taster_id);
        ...
        $time_two = date('A', strtotime($end_time));
        $nextDay = date('Y-m-d', strtotime('+1 day', strtotime($tasting_date)));
        $dateArray = array($nextDay, $tasting_date, $today);

		if($user_type == 'tester'){

            $count = 0;
            if(($end_time <= '03:00') && ($tasting_date > $today)){
			...
			$result=$this->db->get()->row();
            $total_count = $count + $result->count;
     
            if($result->count ==0){
				return 0;
			}else if($total_count ==1 && $result->id == $jobid){
				return 0;
			}else{
				return 1;
			}
    ...
    function get_accepted_joblist_old($tester_id,$todays_job)
    {
        $previous_date=date('Y-m-d', strtotime('-2 months'));
        $todays_date=date("Y-m-d");
