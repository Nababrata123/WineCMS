...
class Notifications_model extends CI_Model
{
	...
	public function send_early_job_notifications($userDetails,$job_id,$testerJobCount)
	{
		if($testerJobCount > 1){
			$body="You have jobs today.";
		}else{
			$body="You have a job today.";
		}
		
		$array=array("title"=>"Job Notification","body"=>$body,"job_id"=>$job_id);

		$res = $this->push_notifications->send_early_job_notifications($userDetails,$array);
		if($res)
		{
			return true;
		}
    ...
    public function job_notifications_between_two_hour($userDetails,$job_id,$start_time)
    {
        $start_time_display=date('h:i A', strtotime($start_time));
        $body="Today you have a job at ".$start_time_display;
		$array=array("title"=>"Job Notification","body"=>$body,"job_id"=>$job_id);

		$res = $this->push_notifications->job_notifications_between_two_hour($userDetails,$array);
		if($res)
		{
			return true;
		}
