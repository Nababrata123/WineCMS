...
class Store_model extends CI_Model {
    ...
    function __construct() {
        ...
        $this->column_search = array('name','sales_rep_name','city','adress','account_number','status');
    ...
    public function get_store_list_for_csv($items_to_export)
    {
        ...
        foreach($store_result as $val){

            $flavour=$val['product_type'];
            $flavour_array=explode('/',$flavour);

            $val['product_type_royal']='No';
            $val['product_type_myx']='No';
            $val['product_type_kayco']='No';
            $val['product_type_other']='No';
            
            foreach ($flavour_array as $flavour) {

                if ($flavour == 'royal') {
                    $val['product_type_royal']='Yes'; 
                }
                 if ($flavour == 'mix'){
                    $val['product_type_myx']='Yes'; 
                }
                if ($flavour == 'kayco'){
                    $val['product_type_kayco']='Yes'; 
                }
                if ($flavour == 'other'){
                    $val['product_type_other']='Yes';
                }
            }
        
            unset($val['product_type']);
	...
	public function get_Sales_Rep_Name($ids){
		$id=explode('#',$ids);
		$name=array();
		foreach($id as $i){
			$this->db->select('*');
    ...
    private function _get_datatables_query($postData,$term=''){
        ...
        //echo $term;die;
        // $this->db->select('*'); 
        $this->db->select("store.id, store.name, store.adress, store.suite_number, store.appartment_number, store.city, store.state,  store.zipcode, store.phone, store.email, store.store_manager, store.sales_rep, store.account_number, store.special_request, store.zone, store.latitude, store.longitude, store.logo, store.status, store.sales_rep_name");
        $this->db->from('store');
