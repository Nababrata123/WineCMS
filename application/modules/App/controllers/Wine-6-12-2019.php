...
class Wine extends Application_Controller {
    ...
    public function add() {
        ...
        if ($this->input->server('REQUEST_METHOD') === 'POST')
        {
            ...
            if ($this->form_validation->run())
            {
                ...
                $check=$this->Wine_model->check_duplicate_upccode($upc_code,$id=null);
                if($check > 0)
                {
                ...
                {*/
                    ...
                    if ($wine_id = $this->Wine_model->insert($this->tablename, $data)) {
                        ...
                        $this->session->set_flashdata('message', '<strong>Well done!</strong> Wine been added successfully.');
                    } else {
                        $this->session->set_flashdata('message_type', 'danger');
                ...
                if(is_uploaded_file($_FILES['file']['tmp_name'])){
                    ...
                    if(!empty($csvData)){
                        ...
                        foreach($csvData as $row){ $rowCount++;
                           // echo $row['company_type'];die;
                            // Prepare data for DB insertion
                            if($row['company_type']=="both")
                            {
                                $company_type='kayco,other';
                            }
                            else
                            {
    ...
    public function view_details()
    {
        ...
        $this->db->select('bottles_sampled,bottles_sold');
