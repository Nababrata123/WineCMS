...
class Category extends Application_Controller {
    ...
    public function delete($id = null) {
		...
		$data_to_store = array(
      	...
      	if ($this->Category_model->update($this->tablename, 'id', $id, $data_to_store)) {
            $this->session->set_flashdata('message_type', 'success');
    		$this->session->set_flashdata('message', '<strong>Well done!</strong> Size successfully deleted.');
        } else {
            $this->session->set_flashdata('message_type', 'danger');
            $this->session->set_flashdata('message', '<strong>Oh snap!</strong> Change something and try again.');
        }
    ...
    public function update_status() {
    	...
    	if ($this->input->server('REQUEST_METHOD') === 'POST')
    	{
    		...
    		if ($this->form_validation->run())
    		{
    			...
    			$this->session->set_flashdata('message_type', 'success');
    			$this->session->set_flashdata('message', '<strong>Well done!</strong> '.$count.' size(s) successfully '.$msg);
