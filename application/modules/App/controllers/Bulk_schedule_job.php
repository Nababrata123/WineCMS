                ...
                ***/
                ...
                while($start_date <= $end_date) {
                    ...
                    if($start_date <= $end_date)
                    {
                        ...
                        // if bulkschedule is create current date Or next date Then send mail (Without taster)....
                    $currentDate = date('Y-m-d');
                    $nextDate = date('Y-m-d', strtotime(' +1 day'));
                    $next2Date = date('Y-m-d', strtotime(' +2 day'));

                    if($insert){
                        if(($taster == '' && $tasting_date == $currentDate) || ($taster == '' && $tasting_date == $nextDate) || ($taster == '' && $tasting_date == $next2Date)){

                                //get next two days schedule 
                                $this->db->select('*');
                                $this->db->from('bulk_schedule_job');
                                $this->db->where('bulk_schedule_job.is_deleted',0);
                                $this->db->where('bulk_schedule_job.id',$insert);
                            
                                $this->db->where('bulk_schedule_job.status','not_published');
                                $result=$this->db->get()->row();


                                $job_date=$result->tasting_date;
                        ...
                        // End Bulkschedule Current date and Next date Send Mail
