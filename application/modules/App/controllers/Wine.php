...
class Wine extends Application_Controller {
    ...
    public function add() {
        ...
        $wine_UOM = $this->Wine_model->getAll_WineUOM();
        $data['wine_size'] = array_filter($wine_UOM);
    ...
    public function edit($id = 0) {
        ...
        // print "<pre>"; print_r($data['sizes']); die;
        $wine_UOM = $this->Wine_model->getAll_WineUOM();
        $data['wine_size'] = array_filter($wine_UOM);
        // print "<pre>"; print_r($data['wine_size']); die;
        $search['parent_id'] = '0';
        $search['deleted'] = 0;
        $search['tag'] = "&ndash;";
        $this->load->model('Category_model');
                    ...
                    $csvData = $this->csvreader->parse_csv($_FILES['file']['tmp_name']);
                     ...
                     if(!empty($csvData)){
						...
						foreach($csvData as $row){
                            ...
                            if (array_key_exists(" id", $row) && array_key_exists("upc_code", $row) && array_key_exists("name", $row) && array_key_exists("brand", $row) && array_key_exists("year", $row) && array_key_exists("type", $row) && array_key_exists("description", $row) && array_key_exists("size", $row) && array_key_exists("UOM", $row) && array_key_exists("category_id", $row) && array_key_exists("company", $row)) {
                        ...
                        if(!$isInvalidKey) {
                            ...
                            // Invalid Collum Alert....
                            $this->session->set_flashdata('message_type', 'danger');
                            $this->session->set_flashdata('message', '<strong>Oh snap!</strong> An invalid file was uploaded. Please select wines from the product list and click export to CSV in order to get the correct CSV format.');
                        }
                    }
                }else{
    ...
    public function images_delete($product_id = null) {
        ...
        $wine_data = array(
            'updated_on' => $cuurentDate
        );

        if ($this->Wine_model->update('wine_images', 'id', $id, $data_to_store)) {

            $this->Wine_model->update('wine', 'id', $product_id, $wine_data);
