...
class Job extends Application_Controller {
    ...
    // Permission Checking
        ...
        $default_uri = array( 'page','status','tasting_date','taster','store','entry_date','sales_rep','search_text');
       ...
       // echo $total_rows;die;
        $config = $this->init_pagination('App/Job/index/'.$this->uri->assoc_to_uri($pegination_uri).'/page/',19, $total_rows);
       // echo "<pre>";
       // print_r($config);die;
    	...
    	if ($this->input->server('REQUEST_METHOD') === 'POST')
    	{
    		...
    		if($this->form_validation->run())
    		{
              ...
              // echo "Start time: ".$start_time;
               ...
               //echo date("H:i", strtotime($this->input->post('start_time')));
                //echo "<br>";
                //echo date("H:i", strtotime($this->input->post('end_time')));die;
              //echo $start_time."<br>".$end_time;die;
               ...
               // echo $check_tester."<br>";
                //echo $check_store."<br>";die;
                $taster_note = $this->input->post('taster_note');
                $wine_details_array=$this->input->post('wine_id');
                //$wine_details_array=$this->session->userdata('wine_ids');
                 //echo "<pre>";
                //print_r($wine_details_array);die;
               ...
               // print_r($wine_details_array);die;
                ...
                if(!empty($wine_details_array))
                {
                    ...
                    {
                        //$wine_id.=$wine_details_array[$i]['wine_id'].",";
                        ...
                        if($check_tester==1)
                        {
                            ...
                            if($count_job == 0)
                            {
                                ...
                                if($insert)
                                {
                                    ...
                                    {
                                        $notifications = $this->Notifications_model->send_notifications_for_republish_job($id,$insert);
                                    }
                                    foreach($tester_array as $tester_id)
                                    {
           ...
           // $this->form_validation->set_rules('question_id[]', 'Question', 'trim|required');
            ...
            if ($this->form_validation->run())
            {
               ...
               $wine_array=$this->input->post('wine_id[]');
                ...
                //Check job for same store and same time
                //$check_store=$this->Job_model->check_store('job',$this->input->post('store_id'),$start_time,$end_time,$tasting_date,$job_id);
    ...
    public function edit_job($job_id=0)
    {
        ...
        //echo date("h:i A", strtotime($pre_start_time));die;
        ...
        if ($this->input->server('REQUEST_METHOD') === 'POST')
        {
            ...
            if ($this->form_validation->run())
            {
                ...
                if($data['job_info']->status=='assigned' || ($data['job_info']->status=='approved' && $data['job_info']->job_state < 1))
                {
                    $status='assigned';
                    $job_status='2';
                }else
            ...
            //print_r($job);die;
                if($end_time < $start_time && $data['job_info']->state!=1)
                {
                    ...
                    }else if($data['job_info']->job_status== 3){
                        redirect('App/Job/index/status/accepted');
                    }else if($data['job_info']->job_status== 2){
                    ...
                    $this->load->library('mail_template');
                       ...
                       // print_r($result['tester_info']);die;
                        ...
                        if($taster_id==$pre_taster && $pre_start_time==$currentjobData->start_time && $pre_end_time!=$currentjobData->end_time){
                            ...
                            $this->mail_template->update_job_email($name,$email, $pre_tasting_date, $pre_start_time, $pre_end_time, $pre_store_data, $currentjobData, $currentStoreData);
                            $notifications = $this->Notifications_model->send_notifications_for_change_info($pre_taster,$job_id);
        ...
        if ($this->input->server('REQUEST_METHOD') === 'POST')
        {
           ...
           // $this->form_validation->set_rules('question_id[]', 'Question', 'trim|required');
            ...
            if ($this->form_validation->run())
            {
                ...
                //Get wine
                $wine_array=$this->input->post('wine_id[]');
                //$wine_array=$this->session->userdata('wine_ids');
                //Check wine type
                $wine_flavours=$this->get_wine_flavour($wine_array);
                if($wine_flavours)
                {
