...
class Sales_representative extends Application_Controller {
     	...
     	if ($this->input->server('REQUEST_METHOD') === 'POST')
     	{
	    	...
	    	foreach ($data['user_meta'] as $fields) {
     		...
     		if ($this->form_validation->run())
     		{
				...
				$result = $this->Sales_representative_model->check_duplicate_email_with_id($this->input->post('email'),$id);
				if($result == 0){
     			//if the insert has returned true then we show the flash message
    ...
    public function delete($id = null) {
        ...
        {
            $this->session->set_flashdata('message_type', 'danger');
                $this->session->set_flashdata('message', '<strong>Oh snap!</strong> The sales rep is associated with job.');
        }
