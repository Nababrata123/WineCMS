...
class Store extends Application_Controller {
    ...
    public function test()
    {
        ...
        //echo 1;die;
        $data = array();
		//print "<pre>";print_r($this->input->post());print "</pre>";die;
        // Get the stores List
        //$memData=$this->Store_model->get_store_list($this->input->post(), 'id', 'asc');
        $memData=$this->Store_model->get_datatables($this->input->post());
		//print_r($memData);die;
        //$i = $this->input->post('start');
        foreach($memData as $member){
			//echo $member->sales_rep;die;
            $row = array();
           // $i++;
           // $created = date( 'jS M Y', strtotime($member->created));
			...
			//$action='<a class="btn btn-primary btn-xs" href="'.$link.'" title="Edit"><span class="glyphicon glyphicon-edit"></span> Edit</a>&nbsp;<a class="btn btn-danger btn-xs delete_button" href="javascript:void(0)"  title="Delete" data-id="'.$id.'"><span class="glyphicon glyphicon-trash"></span> Delete</a>'.$delete_script.'&nbsp <a class="btn btn-success btn-xs" style="margin-top:2px" href="'.$addlLink.'" title="Add Job"><span class="glyphicon glyphicon-plus"></span> Add Job</a>';
                ...
                if(is_uploaded_file($_FILES['file']['tmp_name'])){
                    ...
                    if(!empty($csvData)){
                        ...
                        foreach($csvData as $row){ $rowCount++;
                            ...
                            if($row['name']=='' || $row['address'] =='' || $row['city'] =='' || $row['state']=='' || $row['zipcode']=='' || $row['zone']=='' || $row['product_type']=='' || $row['sales_rep']=='' || $row['email']==''){
                                $blank_data=true;
                                if($row['zone']==''){
                                    $iszoneBlank=true;
