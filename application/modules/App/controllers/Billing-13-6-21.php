...
class Billing extends Application_Controller {
    ...
    // Permission Checking
        ...
        // print_r($uri);die;
        if (isset($uri['search_text']) && urldecode($uri['search_text']) <> "") {
    ...
    public function get_expenses_brandwise()
    {
        ...
        if($this->input->server('REQUEST_METHOD') === 'POST')
        {
            ...
            if ($this->session->userdata('role')=='brand_wise_users'){
                ...
                $brand_id_array = explode(",",$userMeta[0]->meta_value);
            }
            
            $data['expense_details']=$this->Job_model->get_brandwise_expense($brand, $from_date, $to_date, $taster, $agency, $store, $sales_rep, $wine_type, $size, $month, $brand_id_array);
        ...
        // Get users Brand Name for Report section..
        if ($this->session->userdata('role')=='brand_wise_users'){
            $userMeta = $this->Users_model->get_user_meta($user_Id);
            $brand_id_array = explode(",",$userMeta[0]->meta_value);
            $selected_brand_name =$this->Users_model->get_selected_brand($brand_id_array);
            $data['wine_type'] = $this->Users_model->get_selected_brand_wine($selected_brand_name);
            $data['brand']=$selected_brand_name;
        }else{
