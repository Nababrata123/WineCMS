...
class Tester extends Application_Controller {
    ...
    public function test()
    {
        ...
        foreach($memData as $member){
           ...
           // echo $zone;
            ...
            $row[]=$last_login;
			$row[]=$status;
			$row[]= '<span style ="margin-left:5px;margin-top:10px; background-color:#FF8C00; font-size:12px;" class="label label-warning">'.$this->get_avg_rating($member->id).'</span>';
            $row[]=$details;
            $row[]=$c_name;
            $row[]=$action;
            $data[]=$row;
    ...
    public function add() {
    	...
    	if ($this->input->server('REQUEST_METHOD') === 'POST')
    	{
			...
			if(!$this->input->post('chk')){
				$newdata['rate_per_hour'] = $input_meta['rate_per_hour'];
			}
			...
			if(is_array($data['user_meta'])){
		    	...
		    	foreach ($data['user_meta'] as $key=>$value) {
					...
					if($this->input->post('chk')){
						...
						if($field_name=='tasters_rate')
                        {
                            
                            if($meta['tasters_rate']=='')
                            {
                                $this->form_validation->set_rules($field_name, 'tasters_rate', 'required');
                            }
    	...
    	$data['page'] = 'tester';
    	$data['page_title'] = SITE_NAME.' :: Taster Management &raquo; Add Taster';
	...
	public function edit($id = 0) {
        ...
        $data['taster_name']=$this->Tester_model->get_created_by($id);
     	$data['page'] = 'tester';
    	$data['page_title'] = SITE_NAME.' :: Taster Management &raquo; Edit Taster';
   ...
   private function format_date($date) {
	...
	public function rating_filter() {
        ...
        if ($this->input->server('REQUEST_METHOD') === 'POST')
        {
            ...
            //echo $searchData;die;
            $search_by_rating = $this->input->post('rate');
            //echo $search_by_rating;die;
        ...
        foreach($memData as $member){
            ...
            $name= $member->last_name." ".$member->first_name;
            if($search_by_rating==$this->get_round_rating($member->id) && strpos(strtoupper($name), strtoupper($searchData)) !== false){
            $row[]=$checkop;
            $row[]=$name;
            $row[]=$member->email;
            $row[]=$zone;
	...
	public function get_avg_rating($id=0){
        //$this->db->select('taster_id, agency_taster_id, job.id, AVG(job_rating.rating) as rate');
        $this->db->select('job.taster_id, agency_taster_id, job.id, AVG(job_rating.rating) as rate');
        $this->db->from('job');
        $this->db->where('job.taster_id',$id );
        $this->db->or_where('job.agency_taster_id',$id);
        $this->db->join('job_rating', 'job.id = job_rating.job_id');
        $data = $this->db->get()->row();
        //return $data->rate;
        
        if(round($data->rate, 0)==1){
            return $data->rate='☆';
        }elseif(round($data->rate, 0)==2){
            return $data->rate='☆☆';
        }elseif(round($data->rate, 0)==3){
            return $data->rate='☆☆☆';
        }elseif(round($data->rate, 0)==4){
            return $data->rate='☆☆☆☆';
        }elseif(round($data->rate, 0)==5){
            return $data->rate='☆☆☆☆☆';
        }else{
            return $data->rate='N/A';
        }
    }
    public function get_round_rating($id=0){
        //$this->db->select('taster_id, agency_taster_id, job.id, ROUND(AVG(job_rating.rating),0) as rate');
        $this->db->select('job.taster_id, agency_taster_id, job.id, ROUND(AVG(job_rating.rating),0) as rate');
        $this->db->from('job');
        $this->db->where('job.taster_id',$id );
        $this->db->or_where('job.agency_taster_id',$id);
        $this->db->join('job_rating', 'job.id = job_rating.job_id');
