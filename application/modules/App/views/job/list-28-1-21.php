	...
	<legend><span class="glyphicon glyphicon-filter"></span> Filters</legend>
        ...
        <div class="row">
            ...
            <?php if( $filters['status'] == "problems"){ ?>
                <div class="col-md-2">
                    <div class="form-group" style="width:100%;">
                        <label for="inputName" style="">Status Filter</label><br />
    ...
    <table class="table table-striped table-responsive" width="100%">
            ...
            <?php 
                ...
                <td>
                        ...
                        <?php if($filters['status'] == "problems" && $item->finish_time != '00:00:00'){?> 
                            <input type="checkbox" name="item_id[<?php echo $item->id;?>]" class="checkbox-item" value="Y">
                        <?php }?>

                        <?php if( $item->is_archived!=1 &&  $filters['status'] == "accepted" ){?> 
                            <?php $date_now = date("Y-m-d"); ?> 
                        <?php if ($item->endtime_state==1 && $date_now > $item->tasting_date) { ?>
                            <input type="checkbox" name="item_id[<?php echo $item->id;?>]" class="checkbox-item" value="Y">
                            <?php } else if ( $item->endtime_state < 1 && $item->job_state < 1 ) {?>
                   ...
                   <?php if($filters['status'] == "problems")
                    {
                        ...
                        <?php  }else{ ?>
                            ...
                            <span style="padding: 5px;" class="label label-danger">Not Completed</span>
                ...
                <td>
                    ...
                    <div id="<?php echo $item->id;?>" class="hide">
                        ...
                        <?php if($filters['status'] != "problems"){ ?> 
                        <a class="btn btn-warning btn-xs" href="<?php echo base_url('App/Job/clone_job/'.$item->id);?>" title="Change information" style="margin-top:2px"><span class="glyphicon glyphicon-edit"></span> Clone</a> 
                        <?php } ?>
                        
                        <!-- JObs tag humbager inside button -->
                        <!-- JObs tag humbager inside button -->

                        <?php if(($item->job_status==3 && $item->accept_status==1 && $item->status=='cancelled') || ($item->accept_status==0 && $item->status=='rejected') || ($item->job_status==1 && $item->confirm_status==0)) { ?>
                            
                            <a class="btn btn-info btn-xs" href="<?php echo base_url('App/Job/publish_job/'.$item->id);?>" title="Publish">
                        ...
                        <?php $date_now = date("Y-m-d"); ?>
                        <?php if ($item->endtime_state==1 && $date_now > $item->tasting_date) { ?>
                            <a class="btn btn-danger btn-xs" href="javascript:void(0)" title="Delete" style="margin-top:3px" onclick="deleteJob(<?php echo $item->id; ?>)"> <span class="glyphicon glyphicon-trash"></span> Delete</a>
                            <?php } ?>
                            ...
                            <?php if($date_now > $item->tasting_date){?>
                                ...
                                <?php if($item->endtime_state==1 || $item->endtime_state==2 || $item->endtime_state==4){ ?>
                                    <a class="btn btn-primary btn-xs" href="javascript:void(0)" title="Edit"  onclick="edit_modal(<?php echo $item->id;?>)" style="margin-top:3px; margin-bottom: 2px;"> <span class="glyphicon glyphicon-edit"></span> Edit</a>

                                    <?php }else {?>
                                        <a class="btn btn-primary btn-xs" href="javascript:void(0)" title="Edit"  onclick="alert('The taster is setting up the tasting now, please wait until they start the job in order to edit the job end time')" style="margin-top:3px; margin-bottom: 2px;"> <span class="glyphicon glyphicon-edit"></span> Edit</a>
                                <?php } }
                        ...
                        <?php } else if($item->endtime_state==2 || $item->endtime_state==4 || $item->endtime_state==0){?> 
                            <a class="btn btn-primary btn-xs" href="javascript:void(0)" title="Edit"  onclick="edit_modal(<?php echo $item->id;?>)" style="margin-top:3px; margin-bottom: 2px;"> <span class="glyphicon glyphicon-edit"></span> Edit</a>
                        <?php }else if($item->endtime_state==1){?> 
                            <?php if ($date_now > $item->tasting_date) { ?>
                                <a class="btn btn-primary btn-xs" href="javascript:void(0)" title="Edit"  onclick="edit_modal(<?php echo $item->id;?>)" style="margin-top:3px; margin-bottom: 2px;"> <span class="glyphicon glyphicon-edit"></span> Edit</a> 
                            <?php }?>  
                        <?php } ?>
                        ...
                        <?php 
                            if( $item->is_archived!=1 &&  $filters['status'] == "problems" && $item->job_state == 1){ ?>
                                <?php if($difference == 0){ ?>
									<?php if($item->endtime_state==1){ ?> 
                                <a class="btn btn-primary btn-xs" href="javascript:void(0)" title="Edit"  onclick="alert('The taster is setting up the tasting now, please wait until they start the job in order to edit the job end time')" style="margin-top:3px; margin-bottom: 2px;"> <span class="glyphicon glyphicon-edit"></span> Edit</a>
                        		<?php } else if($item->endtime_state==2 || $item->endtime_state==4 || $item->endtime_state==0){?> 
                            	<a class="btn btn-primary btn-xs" href="javascript:void(0)" title="Edit"  onclick="edit_modal(<?php echo $item->id;?>)" style="margin-top:3px; margin-bottom: 2px;"> <span class="glyphicon glyphicon-edit"></span> Edit</a>
                        		<?php }else if($item->endtime_state==3){?> 
                            	<a class="btn btn-primary btn-xs disabled" title="Edit" style="margin-top:3px; margin-bottom: 2px;"> <span class="glyphicon glyphicon-edit"></span> Edit</a> 
                        		<?php } ?>
                                <?php } }?>

                                <?php if( $filters['status'] == "problems"){ ?>
                                    <a class="btn btn-primary btn-xs" style="margin-top:3px;" href="javascript:void(0)" title="Activity" class="setup_button" onclick="view_setup_modal(<?php echo $item->id;?>)">
                                ...
                                <?php if($filters['status'] == "problems" && $item->ready_for_billing==0){?>
                                        <a class="btn btn-primary btn-xs" style="margin-top:3px;" href="javascript:void(0)" title="Billing"  onclick="problem_one_modal(<?php echo $item->id;?>,<?php echo $item->taster_id;?>)">
                            ...
                            if ($filters['status'] != "problems" && $item->job_state != 2 ||($filters['status'] == "problems" && $item->finish_time != '00:00:00')){ ?>
                                <a class="btn btn-danger btn-xs delete_button" style="margin-top:3px;" href="javascript:void(0)" title="Delete" onclick="deleteJob(<?php echo $item->id; ?>);">
                            ...
                            if ($filters['status'] != "problems" && $item->job_state == 2 ){ ?>
                                ...
                                <a class="btn btn-danger btn-xs delete_button" style="margin-top:3px" href="javascript:void(0)" title="Delete" onclick=" completed_job_details_view_modal(<?php echo $item->id; ?>);">
                                    <span class="glyphicon glyphicon-list-alt"></span> View Details
                                </a>

                                <a class="btn btn-success btn-xs" style="margin-top:3px; margin-bottom: 2px;" target="_blank" href="<?php echo base_url();?>App/billing/download_invoice/<?php echo $item->id;?>" title="View"><span class="glyphicon glyphicon-download"></span> View Invoice</a>
			...
			<tfoot>
                <tr>
                    ...
                    <td colspan="8">
                        With selected
                        <button type="submit" id="dltBtn" name="operation" value="delete" class="btn btn-sm btn-danger" style="margin-top:3px" onclick="return confirm('Are you sure you want to delete the job(s)?')"><span class="glyphicon glyphicon-trash"></span> Delete</button>
                    </td>
                    <?php }
                 ?>
                </tr>
