      ...
      <form>
        ...
        <div class="form-group">
          ...
          <center><legend>Time management</legend></center>
              <?php
                $start_hour=date('h',strtotime($job_details->job_start_time));
                $end_hour=date('h',strtotime($job_details->finish_time));
                $start_minute=date('i',strtotime($job_details->job_start_time));
                $end_minute=date('i',strtotime($job_details->finish_time));

                $check_start_Hour=date('h A',strtotime($job_details->job_start_time));
                $check_end_hour=date('h A',strtotime($job_details->finish_time));
                $check_start_minute=date('i',strtotime($job_details->job_start_time));
                $check_end_minute=date('i',strtotime($job_details->finish_time));
                ...
                $am_pm=date('A',strtotime($job_details->job_start_time));
            ...
            ?>
          <div class="col-md-12">
            <label for="inputPhone" class="col-sm-3 control-label">Job start time:</label>
            <!--<div class="col-sm-6">
          ...
          <div class="col-md-12">
            <label for="inputPhone" class="col-sm-3 control-label">Job end time:</label>
            <!--<div class="col-sm-6">
          ...
          <div class="col-sm-3">
            ...
            <div class="col-sm-3">
              ...
              <?php if($am_pm =='PM'){?> <option value="pm" <?php if(date('A', strtotime($job_details->finish_time))=='PM'){echo "selected";}?>>pm</option> <?php } 
              else {?>
...
$("form").submit(function(e){
            ...
            if(!$("#submit_button").hasClass("disabled"))
            {
                ...
                // {
                    ...
                    if(min < 0){
                      alert( "Warning! Job end time can not be less than job start time.");
                      return false;
                    }else if( exp_amount!='' && exp_reason==''){
                        $('#exp_reason').focus();
                        $('#exp_reason').val('');
                        alert("Warning! Please enter expense reason.");
                        return false;
                      }else if( exp_amount=='' && exp_reason!=''){
                        $('#exp_amount').focus();
                        alert("Warning! Please enter expense amount.");
                        return false;
                      }else{
                      ...
                      if(winevals.length!=0){
                            ...
                            if(hasDuplicate){
                            swal("Oops!", "Duplicate wines selected.", "warning");
                            return false;
                            }else{
                      ...
                      //swal("Oops!", "Start time can not be greater than end time .", "warning");return false;
                        $.ajax({
                          type:'POST',
                          url:"<?php echo base_url(); ?>App/job/create_billing_one/",
                          data: $('form').serialize(),
