<div class="subnav">
	<div class="container-fluid">
    	<h1><span class="glyphicon glyphicon-user"></span> Add Bulk Schedule &raquo; <small></small></h1>
        <div id="sub-menu" class="pull-right">
        	<ul class="nav nav-pills">
        		<li><a href="<?php echo base_url('App/Bulk_schedule_job');?>"><span class="glyphicon glyphicon-user"></span> Bulk Schedule</a></li>
	...
	<div class="col-sm-6">
      	<fieldset>
    		...
    		<div class="form-group">
		  		...
		  		<div class="col-sm-7">
		  			<input type="text" name="start_date" class="form-control datepicker" id="start_date" placeholder="Enter start date" value="" required readonly>
		  			<div class="help-block with-errors"></div>
		  		</div>
		  	</div>
            <div class="form-group">
...
{ echo $allinput['user_id'];}?>">
		  		<div class="col-sm-7">
			  		<select name="user_id"  required class="form-control" onchange="get_store(this.value)">
			  			...
			  			<option value="<?php echo $value['id'];?>" <?php if(isset($allinput['user_id']) && ($allinput['user_id']==$value['id'])){echo "selected";}?> ><?php echo $value['first_name']." ".$value['last_name'];?></option>
			  			<?php
			  				}
			  			?>
               ...
               <input type="hidden" value="<?php if(isset($allinput['store_id'])){echo $allinput['store_id'];}?>" id="hidden_store_id">
		  		<div class="col-sm-7">
			  		<select name="store_id"  required class="form-control" id="store" onchange="get_tester(this.value)">
			  			...
			  			<option value="<?php echo $value->id;?>" <?php if(isset($allinput['store_id']) && ($allinput['store_id']==$value->id)){echo "selected";}?>><?php echo $value->name;?></option>
...
$(document).ready(function(){
    ...
    $("form").submit(function(e){
            ...
            {
                var c=confirm('Are you confirm to create the schedule?');
                if(c==true)
                {
...
function get_store(id)
{
    ...
    $.ajax({
		   type:'POST',
		   url:"<?php echo base_url(); ?>App/job/get_store_for_sales_rep/",
		   data: {sales_rep_id:id,hidden_store_id:hidden_store_id},
		   success:function(data){
		    	$("#store").html(data);
		   }
	});
