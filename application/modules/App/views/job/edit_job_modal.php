              ...
              <?php foreach($store as $value){ ?>
                <?php if($value['id']==$job->store_id){ ?>
                  <input type="hidden" id="store" name="store_id" readonly value="<?php echo $job->store_id;?>" class="form-control">
					...
					<?php } ?>
            <?php }else{?> 
              <select name="store_id"  id="store" required class="form-control" onchange="get_tester(this.value,<?php echo ($job->taster_id > 0 ? $job->taster_id:0);?>,<?php echo $job->wine_id;?>)">
...
$(document).ready(function() {
  ...
  $.ajax({
       type:'POST',
       url:"<?php echo base_url(); ?>App/job/get_wine_by_storeId/",
