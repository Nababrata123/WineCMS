            ...
            <div class="form-group">
		  		...
		  		<div class="col-sm-8">
                    <select name="rules"  required class="form-control">                        
						...
						<option value="biweekly">Bi-weekly schedule</option>
...
{ echo $allinput['user_id'];}?>">
            ...
            <div class="form-group">
				...
				<input type="hidden" value="<?php if(isset($allinput['store_id'])){echo $allinput['store_id'];}?>" id="hidden_store_id">
		  		<div class="col-sm-8">
			  		<select name="store_id"  required class="form-control" id="store" value="" onchange="get_tester_wine(this.value);">
	...
	$("form").submit(function(e){
		...
		if(!$("#submit_button").hasClass("disabled"))
		{
			...
			if(c==true){
				...
				// var startDate = new Date(job_date);
				var currentDate = new Date();
				var jobEndDate = new Date(end_date);
				var diffDate = jobEndDate.getTime()-currentDate.getTime();
				var differenceDate = (diffDate / (1000 * 3600 * 24));
				// alert(diffDate / (1000 * 3600 * 24));
				...
				if($('#start_date').val()==''){
					...
					return false;
				}else if(differenceDate > 364){
					$('#wines').focus();
					swal("Oops!","Bulk schedule is limited up to a year ahead","warning");
					return false;
				}
				else{
					return true;
