...
<option value="salesrep" <?php echo (isset($filter['field']) && ($filter['field'] == 'salesrep') ? 'selected' : ''); ?>>Sales rep</option>
 ...
 <?php
	...
	{  
		...
		// $filter['search_text'] = base64_decode($filter['search_text']);
		// 	$filter['search_text'] = str_replace("'","\'",$filter['search_text']);

		// if ($filter['view'] > 10){
		// 	$filter['search_text'] =  urldecode($filter['search_text']);
		// 	$filter['search_text'] = str_replace("'","\'",$filter['search_text']);
		// }else{
		// 	$filter['search_text'] = base64_decode($filter['search_text']);
		// 	$filter['search_text'] = str_replace("'","\'",$filter['search_text']);
 ...
 <!-- <div class="col-md-3">
      <label style="float:right; margin: 0 0 0 0;" >Search: <input type="text" autocomplete="off" name="search_text" id="search_text" class="Store Address" placeholder="" aria-controls="user-table" value="<?php if(isset($filter['search_text']) && $filter['search_text']!="~"){echo $filter['search_text'];}?>"></label>
      </div>
    </div>
   </div> -->
    ...
    <table class="table table-striped table-responsive" width="100%">
      ...
      <tbody>
        ...
        <tr id="job<?php echo $item->id;?>">
			...
			<td>
				<a class="btn1 btn-success btn-xs" style="font-size: 16px;" target="_blank" href="<?php echo base_url();?>App/billing/download_invoice/<?php echo $item->id;?>" title="View"><span class="glyphicon glyphicon-download"></span></a>
			</td>
			<td>
...
<script type="text/javascript">
	...
	$('#view').on('change', function() {
        ...
        var sort=$('#sort').val();
		var ser = encodeURIComponent(search_text_val);
		// var res = encodeURI(uri);
       
	//    alert(search_text_val);
	//    alert(ser);
		/*
		if(inputField !='' && search_text_val ==''){
            if(sort !=''){
          ...
          }else if(search_text_val !='' && inputField ==''){ 
		...
		var niceURI = ser.replace(/[^a-zA-Z0-9-_]/g, '');
		// alert(niceURI);
