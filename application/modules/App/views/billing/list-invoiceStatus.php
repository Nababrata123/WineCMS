    ...
    <table class="table table-striped table-responsive" width="100%">
      ...
      <tbody>
        ...
        <tr id="job<?php echo $item->id;?>">
			...
			<td>
				<?php  $invoice_status = $this->Job_model->get_invoice_status_forBilling();?>

		<?php if($item->id == $invoice_status->job_id && $invoice_status->status == 1) {?>
			<a class="btn btn-success btn-xs" target="_blank" href="<?php echo base_url();?>App/billing/download_invoice/<?php echo $item->id;?>" title="View" style="border: 3px solid #0e440bcf !important; background-color: green; font-size: 14px; color: #fff;"><span class="glyphicon glyphicon-download"></span></a>
			<?php }else{ ?>	
				<a class="btn btn-success btn-xs" target="_blank" onclick="refreshPage(<?php echo $item->id;?>);" title="View" style="font-size: 15px;"><span class="glyphicon glyphicon-download"></span></a>
				<?php }?>
			</td>
			<td>
...
function refreshPage(jobId){
	// window.open('http://localhost/wine/App/billing/download_invoice/'+jobId, '_blank');
	$.ajax({
           type:'POST',
           url:"<?php echo base_url(); ?>App/billing/open_invoice/"+jobId,
           data: {job_id:jobId},
           success:function(data){
			window.open('<?php echo base_url(); ?>App/billing/download_invoice/'+jobId, '_blank');
            location.reload();
